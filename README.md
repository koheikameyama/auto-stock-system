...
**gapup + weekly-break + PSC（高騰後押し目）の3本柱**

| 項目 | gapup | weekly-break | PSC |
|------|-------|-------------|-----|
| **エントリー** | ギャップアップ（3%）+ 陽線 + 出来高サージ（1.5倍） | 13週高値ブレイク + 週足出来高サージ（1.3倍） | 直近20日+15%急騰後、高値-5%以内で出来高干上がり → 出来高サージ1.5倍+陽線で再加速 |
| **損切り** | ATR×1.0（最大3%） | ATR×1.0（最大3%） | ATR×1.0（最大3%） |
| **利確** | トレーリングストップ（固定利確なし） | トレーリングストップ（固定利確なし） | トレーリングストップ（固定利確なし） |
| **平均保有期間** | 1〜2日 | 3〜5日 | 3〜5日 |
| **ポジション上限** | 最大3（独立カウント） | 最大2（独立カウント） | 最大1（独立カウント） |
...
| 15:20 | gapup-monitor | 当日のギャップアップ銘柄をスキャンし、引け条件付き成行注文で自動エントリー |
| 15:20 | psc-monitor | 高騰後押し目銘柄をスキャンし、引け条件付き成行注文で自動エントリー |

position-monitor は Railway Worker の node-cron で毎分実行。gapup-monitor / psc-monitor は node-cron から 15:20-15:25 に実行。
...
- **エントリー条件**: 直近20日で+15%急騰 + 高値から-5%以内 + 出来高サージ（1.5倍以上）+ 陽線
- **時間帯**: 15:20（1日1回スキャン）
- **注文**: 引け条件付き成行注文（market-on-close）→ 約定後に逆指値SLを別途発注
- **ポジション上限**: 最大1（独立カウント）
...
| パラメータ | gapup | weekly-break | PSC |
|-----------|-------|-------------|-----|
| 損切り（SL） | ATR×1.0（最大3%） | ATR×1.0（最大3%） | ATR×1.0（最大3%） |
| BE発動 | ATR×0.3 | ATR×0.5 | ATR×0.5 |
| トレール幅 | ATR×0.3 | ATR×0.8 | ATR×0.8 |
| タイムストップ（基本） | 3営業日 | 5営業日 | 5営業日 |
| タイムストップ（上限） | 5営業日 | 10営業日 | 7営業日 |

### ジョブ実行基盤

| 基盤 | 用途 |
|------|------|
| Railway Worker (node-cron) | 毎分実行（position-monitor）、15:20実行（gapup/psc/weekly-break-monitor） |
