...
単一指標の評価ではなく、4つのカテゴリの**複合条件（コンフルエンス）**が揃わないと高スコアにならない設計。80点以上の銘柄のみAIレビューに進む。
...
|---------|------|-------------|-----------|
| RSI / ストキャスティクス | 13点 | `TechnicalSummary.rsi` | RSI 30-40 = 13点（反発ゾーン）、40-50 = 9点、50-60 = 6点、<30 = 4点、>70 = 0点 |
| 移動平均線 / 乖離率 | 13点 | `TechnicalSummary.maAlignment` | パーフェクトオーダー+方向一致 = 13点、オーダーのみ = 10点、上昇トレンド = 9点、中立 = 6点、下降 = 0-3点 |
| 出来高の変化（方向性込み） | 9点 | `TechnicalSummary.volumeAnalysis.volumeRatio` + OHLCV | 出来高の量（volumeRatio）× 方向性（accumulation/distribution/neutral）で評価。下表参照 |
...
| サブ項目 | 配点 | データソース | 算出ルール |
|---------|------|-------------|-----------|
| チャートパターン | 18点 | `ChartPatternResult[]` | Sランク買い = 18点、Aランク = 14点、Bランク = 11点、Cランク = 7点、Dランク = 5点、なし = 0点。売りパターン検出時は減点。 |
| ローソク足パターン | 7点 | `PatternResult` | 買いシグナル: strength × 0.07（端数丸め）。売りシグナル: (100-strength) × 0.07。なし = 4点（中立） |
...
- テクニカル満点(35) + パターン完璧(25) + 流動性普通(8) + ファンダ良好(12) = **80点** → AI候補
- テクニカル優秀(33) + パターン良好(23) + 流動性良好(18) + ファンダ最悪(0) = **74点** → A止まり（ファンダが足を引っ張る）
- テクニカル優秀(33) + パターン良好(23) + 流動性良好(18) + ファンダ良好(12) = **86点** → S候補
...
model ScoringRecord {
  ...
  // カテゴリ別スコア
  technicalScore    Int   // 0-35
  patternScore      Int   // 0-25
...
export const SCORING = {
  ...
  VOLUME_DIRECTION: {
    ...
    MIN_DATA_DAYS: 3,
    SCORES: {
      HIGH_VOLUME: { accumulation: 9, neutral: 6, distribution: 3 },
      MEDIUM_VOLUME: { accumulation: 7, neutral: 5, distribution: 3 },
      NORMAL_VOLUME: { accumulation: 5, neutral: 4, distribution: 3 },
    },
  },
...
即死ルールチェック
  ...
  ↓
4カテゴリスコアリング
  → テクニカル(35) + パターン(25) + 流動性(25) + ファンダメンタルズ(15) = 総合スコア
  ↓
ランク判定（S/A/B/C）
  → S+A（不足時B追加）を候補として抽出
...
【カテゴリ別】
  テクニカル: 33/35
    RSI: 13/13（RSI=35、反発ゾーン）
    移動平均: 12/13（パーフェクトオーダー成立）
    出来高変化: 8/9（1.8倍 / 買い集め）
  パターン: 23/25
    チャートパターン: 17/18（逆三尊 / Sランク / 勝率89%）
    ローソク足: 6/7（大陽線）
  流動性: 17/25
    売買代金: 8/10（3.2億円）
    値幅率: 5/8（1.8%）
