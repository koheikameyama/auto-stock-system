...
正の期待値を実現するための**3カテゴリ＋ゲート方式のスコアリングシステム（100点満点）**。

「この銘柄は良い銘柄か？」ではなく「この銘柄に**今入るべきか？**」を測る設計。ゲート（即死ルール）でバイナリに足切りした後、トレンド品質・エントリータイミング・リスク品質の3カテゴリで合計100点満点のスコアを算出する。
...
| 条件 | 点数 |
|------|------|
| 出来高5日MA > 25日MA & CV < 0.5 | 7 |
| 出来高5日MA > 25日MA & CV 0.5-0.8 | 5 |
| 出来高安定しているが減少傾向 | 3 |
| 出来高が不安定（CV > 0.8） | 0 |
...
ゲート判定
  ...
  ↓
3カテゴリスコアリング
  → トレンド品質(40) + エントリータイミング(35) + リスク品質(25) = 総合スコア
  ↓
