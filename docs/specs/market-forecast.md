...
- **平日 15:50 JST**（EODワークフロー内、end-of-day完了後に実行）
- ワークフロー: `cronjob_end-of-day.yml`（cron-job.orgトリガー）
- 休場日・システム停止時はスキップ
...
Google News RSSからリアルタイム取得（DB保存なし）。最大30件表示。
...
| `src/core/news-fetcher.ts` | Google News RSSパーサー |
| `src/jobs/market-forecast.ts` | メインジョブ |
