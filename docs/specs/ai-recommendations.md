...
2. ユーザーごとに並列処理（最大3並列）:
   ...
   e2. ウォッチリスト銘柄を除外（5件以上の候補がある場合のみ）。保有銘柄は追加購入候補として除外しない
...
保有銘柄はおすすめ候補から除外せず、追加購入のチャンスがあれば積極的に推奨する。

- ウォッチリスト銘柄のみ除外対象（5件以上の候補がある場合）
- 保有銘柄が候補に含まれる場合、AIプロンプトに保有情報を伝え「追加購入」の文脈で理由を生成
- AIは保有銘柄を選んだ場合、reason の冒頭に「【追加購入チャンス】」を付与
- フロントエンドでは「買い増しチャンス」バッジを表示（「保有中」の代わり）
