...
> **⛔ 廃止**: この機能は廃止されました。売買推奨がなくなったため、推奨精度の追跡は不要になりました。関連テーブル（`DailyAIReport`, `WeeklyAIReport`）も廃止対象です。
...
詳細は [recommendation-outcomes.md](recommendation-outcomes.md) を参照。
