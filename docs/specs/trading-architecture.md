...
market-scannerの判定は「機械的ゲート → 戦略決定 → AI判断」の順で実行される。機械的ゲートはAIより先に動作し、明確な数値条件で取引を停止する。AIはゲートを通過した場合のみ呼ばれ、定性的判断に集中する。
...
[2] ========== 機械的ゲート（AIより先に実行） ==========
  ...
  │   └─ VIX < 20 → 制限なし
  │
  ├─ [1.8.1] ★ 戦略決定（VIX・CME乖離率ベース）
  │   └─ VIX ≥ 25 → day_trade（持ち越しリスク回避）
  │   └─ CME乖離率 ≤ -1.5% → day_trade（ギャップリスク回避）
  │   └─ それ以外 → swing（デフォルト）
  │
  ├─ [1.8.5] 日経平均キルスイッチ
  │   └─ 前日比 ≤ -3% → 🚫 取引停止（crisis）
...
[3] ========== AI判断 ==========
  │
  ├─ [2] AI市場評価（assessMarket）
  │   └─ shouldTrade = false → シャドウモード（スコアリングのみ）
  │   └─ shouldTrade = true → 銘柄選定へ
  │
  ├─ [3-4] テクニカル分析 + スコアリング → 上位10-20銘柄に絞り込み
  │
  └─ [4] AI銘柄レビュー（reviewStocks）
      └─ Go/No-Go判断（定性的リスク評価のみ）
      └─ 戦略は[1.8.1]で決定済み → AIは変更不可
  ↓
...
| # | 条件 | 閾値 | 種別 | 実装 |
|---|------|------|------|------|
| 1 | CME先物乖離率 | ≤ -3.0% | 機械的 | `determinePreMarketRegime()` |
| 2 | VIX crisis | > 30 | 機械的 | `determineMarketRegime()` |
| 3 | 日経平均急落 | 前日比 ≤ -3% | 機械的 | `MARKET_INDEX.NIKKEI_CRISIS_THRESHOLD` |
| 4 | 週次ドローダウン | -5% | 機械的 | `DRAWDOWN.WEEKLY_HALT_PCT` |
| 5 | 月次ドローダウン | -10% | 機械的 | `DRAWDOWN.MONTHLY_HALT_PCT` |
| 6 | 連敗 | 5連敗 | 機械的 | `DRAWDOWN.COOLDOWN_HALT_TRIGGER` |
| 7 | AI市場評価 | shouldTrade=false | AI判断 | `assessMarket()` |

1-6は機械的・無条件で停止。7はAIの定性判断（ニュース・地政学リスク等）。

#### システムとAIの役割分担

| 判断 | 担当 | 方法 | 備考 |
|------|------|------|------|
| 取引停止 | システム（機械的） | VIX/CME/日経/ドローダウン閾値 | AIより先に実行、AIは上書き不可 |
| 取引見送り | AI | shouldTrade判定 | ニュース・地政学等の定性判断 |
| 戦略決定 | システム（機械的） | VIX/CME乖離率 | AIは選ばない・変更不可 |
| ポジション制限 | システム（機械的） | VIXレジーム | 最大ポジション数・最低ランク |
| 銘柄選定 | システム → AI | スコアリング → Go/No-Go | ロジックが主役、AIが最終審判 |
| エントリー条件 | システム → AI | ロジック算出 → AIレビュー | 損切りはロジック優先 |
| 損切り実行 | システム（機械的） | ATRベース | AIの判断を介さない |
| トレーリングストップ | システム（機械的） | ATRベース | 自動引き上げ |

**原則**: 数値で判断できることはシステムが機械的に処理し、AIには「ロジックが見落としがちな定性的リスク」の判断だけを任せる。

#### AIプロンプトとの整合性

AIプロンプト（`market-assessment.ts`）のセンチメント基準は、システムの機械的ゲートと矛盾しないよう設計されている:

| VIX | システム（レジーム） | システム（戦略） | AIの判断範囲 |
|-----|---------------------|----------------|-------------|
| < 20 | normal（制限なし） | swing | bullish/neutral/bearish |
| 20-25 | elevated（2ポジ・S/A） | swing | bullish/neutral/bearish |
| 25-30 | high（1ポジ・S） | day_trade | neutral（VIXだけではbearishにしない）|
| > 30 | crisis（停止） | - | crisis（機械的にも停止済み） |

- VIX 25-30: システムがポジション制限+デイトレ切り替えで対応済み → AIがVIXだけでbearishにすると二重制限で取引不能になるため禁止
- VIX > 30: システムもAIも停止（冗長だが安全）
- bearish判定: ニュース・地政学・CME乖離等の定性的要因がある場合のみ
...
- **ダッシュボード**: 3ゲート（VIXレジーム・AI市場評価・ドローダウン）の結果を統合した最終判定バッジ + sentimentバッジ
...
```
リスク許容額 = 総資金 × RISK_PER_TRADE_PCT（2%）
...
- `src/core/risk-manager.ts`: `calculatePositionSize()`
- `src/lib/constants/scoring.ts`: `POSITION_SIZING.RISK_PER_TRADE_PCT`
...
| コスト | 計算 |
|--------|------|
| 手数料 | 立花証券e-Supportプラン（ティア制、買い+売りの往復） |
...
**設計思想**: 「損小利大 — 利益を伸ばし、損失を限定する」— トレーリングストップを出口戦略の主軸とし、固定利確は廃止。タイムストップ（10営業日）で塩漬けを防止する。
...
| パラメータ | デイトレ | スイング | 説明 |
|-----------|---------|---------|------|
| アクティベーション閾値 | ATR × 0.5 | ATR × 0.5 | エントリー価格からの上昇幅（より早く発動） |
| トレール幅 | ATR × 1.0 | ATR × 1.2 | 最高値からの距離（やや引き締め） |
| フォールバック発動% | 1.0% | 1.0% | ATR不明時 |
| フォールバックトレール% | 1.5% | 2.0% | ATR不明時 |
...
| パラメータ | デイトレ | スイング | 説明 |
|-----------|---------|---------|------|
| BE発動閾値 | ATR × 0.8 | ATR × 1.0 | ブレイクイーブン発動までの上昇幅 |
| TS発動閾値 | ATR × 1.2 | ATR × 2.0 | トレーリングストップ発動までの上昇幅 |
...
スイングトレードの保有期間を最大10営業日に制限する。期間内にトレーリングストップが発動しなかった場合、損益に関わらず成行で決済する。トレンドフォロー戦略において、10日経っても動かない銘柄はエントリーの前提（トレンド継続）が崩れていると判断する。
