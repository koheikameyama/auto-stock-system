...
VIXは米国市場の指標であり、日本株の急落をリアルタイムに反映しきれない。日経平均の前日比が **-3%以下** の場合、VIXレジームに関わらずCrisisモード（全取引停止＋全ポジション即時決済）に自動移行する。
...
|------|-----------|-------------|------|
| **未発動** | 固定SL | なし（固定TP廃止） | SLのみ監視 |
| **発動後** | トレーリングストップ | なし | 上値を追う |
