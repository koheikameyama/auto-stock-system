...
市場環境に基づいてday_trade/swingを**日単位・全銘柄共通**で決定する。ルールベースで機械的に決定する。
...
market-scanner（朝）:
  ...
  4. 市場評価: 固定 "bullish"（機械的ゲートでリスク管理）
  5. フィルター通過銘柄を自動承認
  ...
  2-5. 注文キャンセル → 日次サマリー → 機械的サマリー → Slack通知
```
...
- **銘柄選定**: フィルター通過銘柄は自動承認
- **リスク管理**: VIXレジーム・キルスイッチ・ドローダウン管理で機械的に実行
...
| 戦略決定             | VIX/CME乖離率                 | ルールベースで機械的に決定   |
| ポジション制限       | VIXレジーム                   | 最大ポジション数・最低ランク |
...
| VIX   | レジーム               | 戦略      | 動作                                   |
| ----- | ---------------------- | --------- | -------------------------------------- |
| < 20  | normal（制限なし）     | swing     | 通常取引                               |
| 20-25 | elevated（2ポジ・A）   | swing     | ポジション・スコア制限                 |
| 25-30 | high（1ポジ・S）       | day_trade | ポジション制限+デイトレ切替            |
| > 30  | crisis（1ポジ・S）     | day_trade | 1ポジション制限+EODスイング強制決済    |
