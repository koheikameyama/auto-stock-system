...
| パラメータ              | デイトレ  | スイング  | 説明                       |
| ----------------------- | --------- | --------- | -------------------------- |
| アクティベーション閾値  | ATR × 1.2 | ATR × 2.5 | エントリー価格からの上昇幅 |
| トレール幅              | ATR × 0.8 | ATR × 1.5 | 最高値からの距離           |
| フォールバック発動%     | 1.5%      | 4.0%      | ATR不明時                  |
| フォールバックトレール% | 1.0%      | 4.0%      | ATR不明時                  |
...
| パラメータ | デイトレ  | スイング  | 説明                                 |
| ---------- | --------- | --------- | ------------------------------------ |
| BE発動閾値 | ATR × 0.8 | ATR × 1.5 | ブレイクイーブン発動までの上昇幅     |
| TS発動閾値 | ATR × 1.2 | ATR × 2.5 | トレーリングストップ発動までの上昇幅 |
...
スイングトレードの保有期間を最大10営業日に制限する。ただし、**トレーリングストップが発動中のポジションにはタイムストップを適用しない**（利益を伸ばすためトレーリングに委ねる）。トレーリング未発動のまま10営業日が経過した場合、損益に関わらず成行で決済する。トレンドフォロー戦略において、10日経っても動かない銘柄はエントリーの前提（トレンド継続）が崩れていると判断する。
