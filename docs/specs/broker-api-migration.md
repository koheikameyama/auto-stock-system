...
### 日経VI（ボラティリティ・インデックス）の取得

**Yahoo Finance の `^JNV` シンボルがデータを返さなくなっている（2026-03時点）。**

立花証券APIは個別銘柄（日本株）のリアルタイム株価・板情報・日足が対象で、日経VIなどの市場指数は対応していない可能性が高い。

**現状の対処（VIXフォールバック）:**
- `nikkeiVi` が `null` の場合、AIプロンプトに「取得不可」と表示
- VIXを代替指標として使用（VIX→日経VI換算基準をプロンプトに定義済み）
- 機械的レジーム判定も `determineMarketRegimeFromVix()` にフォールバック

**移行時の対応案:**
1. **ETN 2035.T（NEXT NOTES 日経VI先物ETN）** — 立花証券APIで個別銘柄として取得可能。日経VIそのものではないが、変化率（前日比%）は方向性の代替指標になる
2. **JPX有料データサービス** — 日経VIの公式データを提供しているが、コストがかかる
3. **VIXフォールバック維持** — 現状の実装で安全側に倒れるため、日経VIが取れなくても運用上問題ない
