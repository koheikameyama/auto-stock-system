...
**現状の問題**: position-monitorに9:00-9:30の「寄付き乱高下回避」フィルタがあるが、Yahoo Finance 20分遅延により事実上機能していない。

- order-managerは9:20に実行 → Yahoo Finance経由で受け取るのは**9:00時点のデータ**
- position-monitorの9:00-9:30ブロックは、注文作成（9:20）後の**残り10分しか機能しない**
- 9:30にブロック解除されても、20分遅延で利用可能なのは**9:10時点のデータ**（まだ寄付き直後）
- Yahoo Financeの`quote.high`/`quote.low`は**当日の累積値**のため、寄付きの急落は終日lowに残り続ける

→ 結果として、寄付き回避フィルタは建前だけで何も守れていない。
