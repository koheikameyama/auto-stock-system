...
スコアリング+AIレビュー方式は廃止済み（コードは削除、git履歴で復元可能）。
...
## 2. Midday Reassessment（src/jobs/midday-reassessment.ts）
...
## 6. JPX CSV同期（src/jobs/jpx-csv-sync.ts）
...
## 7. JPX廃止予定同期（src/jobs/jpx-delisting-sync.ts）
...
## 8. Backfill Prices（src/jobs/backfill-prices.ts）
...
利用可能: news / scan / monitor / eod / weekly / backfill
