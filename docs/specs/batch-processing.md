...
| morning-sl-sync | cronjob_morning-sl-sync.yml | 平日 8:50 JST | SL注文の朝次再発注・トレーリングストップ回復（市場オープン前） |
...
**役割**: 本番環境のみ。市場オープン前にオープンポジションのSL注文を再発注し、トレーリングストップを最新化する。

### 背景

本番環境でのポジション保護を目的としたジョブ。市場オープン（9:00）前にDBのオープンポジションに対してSL注文を再発注し、前日からのトレーリングストップ追従分を回復する。デモ環境では `isTachibanaProduction` が false となるため、処理を即時スキップする。
...
| 環境 | 挙動 |
|------|------|
| `TACHIBANA_ENV=production` | SL注文再発注・トレーリングストップ回復を実行 |
| `TACHIBANA_ENV=demo` | 即時リターン（処理スキップ） |

### 備考

- デモ環境は `isTachibanaProduction` が false のため、ジョブ冒頭で即時リターンする（21〜24行目）
