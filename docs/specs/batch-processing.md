...
| order-manager | cronjob_order-manager.yml | 平日 9:50 JST | 注文生成（寄付き後30分のデータ反映、寄り付きセッション分析込み） |
...
**実行**: 平日 9:50 JST
**役割**: 寄り付きセッション分析を含むAI判断に基づき取引注文を生成する
...
## 5.5. Holding Score（market-scanner内で実行）

**実行**: market-scannerの冒頭で実行（平日 8:00 JST）
**役割**: オープンポジションの銘柄を日次スコアリングし、トレンド劣化時にトレーリングストップを引き締める
