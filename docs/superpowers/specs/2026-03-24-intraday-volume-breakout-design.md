...
interface WatchlistEntry {
  ...
  high20: number;           // 20日高値（価格ブレイク判定の基準）
...
**実行環境**: workerプロセス内のnode-cronジョブ

**ティアード・ポーリング**:

| ティア | 対象 | 頻度 | API呼び出し |
|---|---|---|---|
| Cold | ウォッチリスト全体 (~90銘柄) | 5分間隔 | ~1,620 calls/時 |
| Hot | 出来高急増候補 (~5-10銘柄) | 1分間隔 | ~600 calls/時 |
...
elapsedFraction = ザラ場経過分数 / 300（総取引時間300分）
```

例: 9:30時点（30分経過）、avgVolume25 = 100,000株
