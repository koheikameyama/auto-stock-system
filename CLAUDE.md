...
- **出口戦略**: トレーリングストップ一本で利益を伸ばし、タイムストップ（10営業日）で塩漬けを防止
- **損切り**: ATRベースで機械的に損切り（最大3%）、固定利確は廃止
- **頻度**: スイングトレード主体（保有期間3〜10営業日）
...
- **AIの役割**: トレンドに合致したエントリーポイント検出、定性リスク判断、Go/No-Go最終審判
- **ルールベース**: 損切りはATRベースで機械的に実行、利確はトレーリングストップに委ねて利益を伸ばす

#### 実装ガイドライン

- エントリー条件はトレンド方向の複数指標が一致した場合のみ（RR ≥ 1.5必須）
- 損切りは必ず設定し、例外なく実行する（最大3%）
- 固定利確は使わない — トレーリングストップで利益を伸ばす
- 10営業日経過でクローズ（タイムストップ）
- バックテストで期待値 > 0、PF ≥ 1.3 を確認してから本番適用する
