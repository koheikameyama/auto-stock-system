...
実際のロジックは /api/highlights/generate-daily に実装済み。
"""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response = requests.post(
                f"{app_url}/api/highlights/generate-daily",
                headers={
...
def main():
    ...
    print("=" * 60)
    print("Daily Highlights Generation (Python -> TypeScript API)")
    print("=" * 60)
    print(f"Time: {datetime.now().isoformat()}")
    print(f"Session: {session}")
    print(f"API URL: {app_url}/api/highlights/generate-daily")
    print()
