// scripts/walk-forward-ddr.ts
/**
 * Down-Day Reversal Walk-Forward 分析
 *
 * IS（In-Sample）6ヶ月 / OOS（Out-of-Sample）3ヶ月
 * 3ヶ月スライド × 7ウィンドウ = 27ヶ月
 *
 * Usage:
 *   npm run walk-forward:ddr
 */
...
import { runDdrBacktest, precomputeDdrDailySignals } from "../src/backtest/down-day-reversal-simulation";
import { DDR_BACKTEST_DEFAULTS, generateDdrParameterCombinations } from "../src/backtest/down-day-reversal-config";
import type { DownDayReversalBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<DownDayReversalBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<DownDayReversalBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<DownDayReversalBacktestConfig>): string {
...
async function main() {
  ...
  console.log("Down-Day Reversal Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateDdrParameterCombinations();
  ...
  const maxPrice = DDR_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = DDR_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeDdrDailySignals(filterCfg, allData, isPrecomputed);

    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: DownDayReversalBacktestConfig = {
        ...DDR_BACKTEST_DEFAULTS,
        ...params,
        startDate: isStart,
        endDate: isEnd,
        verbose: false,
      ...
      const result = runDdrBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeDdrDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: DownDayReversalBacktestConfig = {
      ...DDR_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runDdrBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
