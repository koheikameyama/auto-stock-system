// scripts/walk-forward-gapup.ts
/**
 * ギャップアップ戦略 Walk-Forward 分析
 *
 ...
 *   npm run walk-forward:gapup -- --max-pf
 */
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<GapUpBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<GapUpBacktestConfig>;
...
async function main() {
  ...
  console.log("GU（ギャップアップ）戦略 Walk-Forward 分析");
  console.log("=".repeat(70));
