/**
 * モメンタム戦略 Walk-Forward 分析
 *
 * IS（In-Sample）6ヶ月 / OOS（Out-of-Sample）3ヶ月
 * 3ヶ月スライド × 6ウィンドウ = 24ヶ月
 *
 * Usage:
 *   npm run walk-forward:momentum
 *   npm run walk-forward:momentum -- --max-pf
 */
...
const MIN_IS_PF = 0.5;
/** モメンタムはトレード数が少ないため閾値を緩和 */
const MIN_IS_TRADES = 3;
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<MomentumBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<MomentumBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<MomentumBacktestConfig>): string {
...
async function main() {
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeMomentumSignals(filterCfg, allData, isPrecomputed);
    ...
    const oosResult = runMomentumBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
