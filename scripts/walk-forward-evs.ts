// scripts/walk-forward-evs.ts
/**
 * Early Volume Spike Walk-Forward 分析
 *
 * IS（In-Sample）6ヶ月 / OOS（Out-of-Sample）3ヶ月
 * 3ヶ月スライド × 7ウィンドウ = 27ヶ月
 *
 * Usage:
 *   npm run walk-forward:evs
 */
...
import { runEvsBacktest, precomputeEvsDailySignals } from "../src/backtest/early-volume-spike-simulation";
import { EVS_BACKTEST_DEFAULTS, generateEvsParameterCombinations } from "../src/backtest/early-volume-spike-config";
import type { EarlyVolumeSpikeBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<EarlyVolumeSpikeBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<EarlyVolumeSpikeBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<EarlyVolumeSpikeBacktestConfig>): string {
...
async function main() {
  ...
  console.log("Early Volume Spike Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  if (budget != null) console.log(`初期資金: ¥${budget.toLocaleString()}`);

  const paramCombos = generateEvsParameterCombinations();
  ...
  const maxPrice = EVS_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = { ...EVS_BACKTEST_DEFAULTS, ...(budget != null ? { initialBudget: budget } : {}) };
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeEvsDailySignals(filterCfg, allData, isPrecomputed);

    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: EarlyVolumeSpikeBacktestConfig = {
        ...filterCfg,
        ...params,
        startDate: isStart,
        endDate: isEnd,
      ...
      const result = runEvsBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeEvsDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: EarlyVolumeSpikeBacktestConfig = {
      ...filterCfg,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runEvsBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
