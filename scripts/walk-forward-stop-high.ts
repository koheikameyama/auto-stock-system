// scripts/walk-forward-stop-high.ts
/**
 * ストップ高フォロースルー Walk-Forward 分析
 *
 * IS（In-Sample）6ヶ月 / OOS（Out-of-Sample）3ヶ月
 * 3ヶ月スライド × 7ウィンドウ = 27ヶ月
 *
 * Usage:
 *   npm run walk-forward:stop-high
 */
...
import { runStopHighBacktest, precomputeStopHighDailySignals } from "../src/backtest/stop-high-simulation";
import { STOP_HIGH_BACKTEST_DEFAULTS, generateStopHighParameterCombinations } from "../src/backtest/stop-high-config";
import type { StopHighBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<StopHighBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<StopHighBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<StopHighBacktestConfig>): string {
...
async function main() {
  ...
  console.log("ストップ高フォロースルー Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateStopHighParameterCombinations();
  ...
  const maxPrice = STOP_HIGH_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = STOP_HIGH_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeStopHighDailySignals(filterCfg, allData, isPrecomputed);

    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: StopHighBacktestConfig = {
        ...STOP_HIGH_BACKTEST_DEFAULTS,
        ...params,
        startDate: isStart,
        endDate: isEnd,
        verbose: false,
      ...
      const result = runStopHighBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeStopHighDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: StopHighBacktestConfig = {
      ...STOP_HIGH_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runStopHighBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
...
main().catch((err) => {
  console.error("Stop-High WFエラー:", err);
  process.exit(1);
});
