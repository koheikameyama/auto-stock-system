...
async function main() {
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    console.log(`  最適パラメータ: trigger=${bestParams.triggerThreshold}x, high=${bestParams.highLookbackDays}日, ` +
      `atr=${bestParams.atrMultiplier}, trail=${bestParams.trailMultiplier}, ts=${bestParams.tsActivationMultiplier}`);
    console.log("");
  }
...
function printSummary(results: WindowResult[]): void {
  ...
  for (const r of results) {
    ...
    const paramStr = `trig=${p.triggerThreshold} high=${p.highLookbackDays} atr=${p.atrMultiplier} trail=${p.trailMultiplier} ts=${p.tsActivationMultiplier}`;
    console.log(
  ...
  // パラメータ安定性分析
  console.log("\n[パラメータ安定性]");
  const paramKeys = ["triggerThreshold", "highLookbackDays", "atrMultiplier", "trailMultiplier", "tsActivationMultiplier"] as const;
