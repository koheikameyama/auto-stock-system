// scripts/walk-forward-ogf.ts
/**
 * Overnight Gap-Fade Walk-Forward 分析
 *
 * IS（In-Sample）6ヶ月 / OOS（Out-of-Sample）3ヶ月
 * 3ヶ月スライド × 7ウィンドウ = 27ヶ月
 *
 * Usage:
 *   npm run walk-forward:ogf
 */
...
import { runOvernightGapFadeBacktest, precomputeOgfDailySignals } from "../src/backtest/overnight-gap-fade-simulation";
import { OGF_BACKTEST_DEFAULTS, generateOgfParameterCombinations } from "../src/backtest/overnight-gap-fade-config";
import type { OvernightGapFadeBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<OvernightGapFadeBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<OvernightGapFadeBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<OvernightGapFadeBacktestConfig>): string {
...
async function main() {
  ...
  console.log("Overnight Gap-Fade Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateOgfParameterCombinations();
  ...
  const maxPrice = OGF_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = OGF_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeOgfDailySignals(filterCfg, allData, isPrecomputed);

    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: OvernightGapFadeBacktestConfig = {
      ...
      const result = runOvernightGapFadeBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeOgfDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: OvernightGapFadeBacktestConfig = {
      ...OGF_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runOvernightGapFadeBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
