// scripts/walk-forward-nr7.ts
/**
 * NR7ブレイク（Narrow Range 7）Walk-Forward 分析
 *
...
import { runNR7Backtest, precomputeNR7DailySignals } from "../src/backtest/nr7-simulation";
import { NR7_BACKTEST_DEFAULTS, generateNR7ParameterCombinations } from "../src/backtest/nr7-config";
import type { NR7BacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<NR7BacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<NR7BacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<NR7BacktestConfig>): string {
...
async function main() {
  ...
  console.log("NR7ブレイク（Narrow Range 7）Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateNR7ParameterCombinations();
  ...
  const maxPrice = NR7_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = NR7_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeNR7DailySignals(filterCfg, allData, isPrecomputed);

    // IS: 全パラメータ組み合わせテスト
    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: NR7BacktestConfig = {
        ...NR7_BACKTEST_DEFAULTS,
        ...params,
        startDate: isStart,
        endDate: isEnd,
        verbose: false,
      ...
      const result = runNR7Backtest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeNR7DailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: NR7BacktestConfig = {
      ...NR7_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runNR7Backtest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
