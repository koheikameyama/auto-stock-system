...

{
  ...
  //   suggestedStopLossRate: 推奨損切り率（0.01〜0.30）
  //   suggestedTakeProfitRate: 推奨利確率（0.05〜1.00）
  "styleAnalyses": {
    "CONSERVATIVE": { ...上記フィールド（損切り率は狭め、利確率は控えめ） },
    "BALANCED": { ...上記フィールド },
    "AGGRESSIVE": { ...上記フィールド（損切り率は広め、利確率は高め） }
  },
...
【損切り率・利確率の算出指針】
- 購入した場合に設定すべき損切り率と利確率を、スタイルごとに提案
- 高ボラティリティ → 損切り率を広めに、低ボラティリティ → 狭めに
- スタイル別の目安:
