/**
 * Overnight Gap-Fade バックテスト・シミュレーションエンジン
 *
 * 日次ループで Overnight Gap-Fade シグナルを検出し、当日終値でエントリー。
...
import { OGF_RISK_PER_TRADE_PCT } from "./overnight-gap-fade-config";
import { isOvernightGapFadeSignal } from "../core/overnight-gap-fade/entry-conditions";
...
import type {
  OvernightGapFadeBacktestConfig,
  OvernightGapFadeBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
...
/**
 * Overnight Gap-Fade シグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedOgfSignal {
  ticker: string;
  entryPrice: number;
  atr14: number;
  prevBodyPct: number;
...
/** entryDate → signals (prevBodyPct 降順) */
export type PrecomputedOgfSignals = Map<string, PrecomputedOgfSignal[]>;

/**
 * Overnight Gap-Fade シグナルを一括事前計算する。
 */
export function precomputeOgfDailySignals(
  config: Pick<OvernightGapFadeBacktestConfig,
    | "maxPrice" | "minAtrPct" | "minAvgVolume25" | "minTurnover" | "minPrice"
    | "prevMinBodyPct" | "minGapDownPct" | "maxGapDownPct" | "minBodyPct" | "prevVolSurgeRatio"
...
): PrecomputedOgfSignals {
  const result: PrecomputedOgfSignals = new Map();
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    // マーケットフィルター（gapupと同じ: breadth + index SMA50）
    ...
    const daySignals: PrecomputedOgfSignal[] = [];
    ...
    for (const [ticker, bars] of allData) {
      ...
      // シグナル判定（前日のデータ + 当日のデータ）
      if (!isOvernightGapFadeSignal({
        prevOpen: prevBar.open,
        prevClose: prevBar.close,
        prevVolume: prevBar.volume,
        todayOpen: todayBar.open,
        todayClose: todayBar.close,
        avgVolume25,
        prevMinBodyPct: config.prevMinBodyPct,
        minGapDownPct: config.minGapDownPct,
        maxGapDownPct: config.maxGapDownPct,
        minBodyPct: config.minBodyPct,
        prevVolSurgeRatio: config.prevVolSurgeRatio,
      })) continue;
      ...
      const prevBodyPct = prevBar.close / prevBar.open - 1;
      const gapPct = todayBar.open / prevBar.close - 1;
      ...
      daySignals.push({
        ...
        entryPrice,
        atr14,
        prevBodyPct: Math.round(prevBodyPct * 10000) / 10000,
    ...
    if (daySignals.length > 0) {
      // prevBodyPct 降順（前日の陽線が強い順）
      daySignals.sort((a, b) => b.prevBodyPct - a.prevBodyPct);
...
/**
 * Overnight Gap-Fade バックテストを実行する
 */
export function runOvernightGapFadeBacktest(
  config: OvernightGapFadeBacktestConfig,
  allData: Map<string, OHLCVData[]>,
  vixData?: Map<string, number>,
  indexData?: Map<string, number>,
  precomputed?: PrecomputedSimData,
  precomputedSignals?: PrecomputedOgfSignals,
): OvernightGapFadeBacktestResult {
  ...
  if (precomputed) {
    ...
    precomputedSignals = precomputeOgfDailySignals(config, allData, computed);
  }
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (todayRegime !== "crisis" && openPositions.length < config.maxPositions) {
      ...
      for (const signal of signals) {
        ...
        const riskAmount = cash * (OGF_RISK_PER_TRADE_PCT / 100);
        ...
        if (config.verbose) {
          console.log(
            ...
            ` (prevBody${(signal.prevBodyPct * 100).toFixed(1)}%, gap${(signal.gapPct * 100).toFixed(1)}%, body${(signal.bodyPct * 100).toFixed(1)}%, SL¥${stopLossPrice})`,
          );
        }
      }
    }
