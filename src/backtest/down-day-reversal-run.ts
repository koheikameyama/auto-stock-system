/**
 * Down-Day Reversal バックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:down-day-reversal
 *   npm run backtest:down-day-reversal -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:down-day-reversal -- --verbose
 */

import dayjs from "dayjs";
import { prisma } from "../lib/prisma";
import { DDR_BACKTEST_DEFAULTS } from "./down-day-reversal-config";
import { getMaxBuyablePrice } from "../core/risk-manager";
import { runDdrBacktest } from "./down-day-reversal-simulation";
...
import type { DownDayReversalBacktestConfig, PerformanceMetrics } from "./types";
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? DDR_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("Down-Day Reversal バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: DownDayReversalBacktestConfig = {
  ...
  const result = runDdrBacktest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "Down-Day Reversal 戦略");
  ...
  try {
    const id = await saveBacktestResult(result, "down-day-reversal");
