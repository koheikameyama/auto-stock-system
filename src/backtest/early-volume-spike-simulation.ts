/**
 * Early Volume Spike バックテスト・シミュレーションエンジン
 *
 * 日次ループで出来高異常サージ（5倍以上）+ 20日高値80-95%位置のシグナルを検出し、
 * 当日終値でエントリー。エグジットは既存の checkPositionExit() を再利用。
 */
...
import { EVS_RISK_PER_TRADE_PCT } from "./early-volume-spike-config";
import { isEarlyVolumeSpikeSignal } from "../core/early-volume-spike/entry-conditions";
...
import type {
  EarlyVolumeSpikeBacktestConfig,
  EarlyVolumeSpikeBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
...
/**
 * Early Volume Spike シグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedEvsSignal {
  ticker: string;
  entryPrice: number;
  atr14: number;
...
export type PrecomputedEvsSignals = Map<string, PrecomputedEvsSignal[]>;

/**
 * Early Volume Spike シグナルを一括事前計算する。
 */
export function precomputeEvsDailySignals(
  config: Pick<EarlyVolumeSpikeBacktestConfig,
    | "maxPrice" | "minAtrPct" | "minAvgVolume25" | "minTurnover" | "minPrice"
    | "volSurgeRatio" | "minHigh20Ratio" | "maxHigh20Ratio" | "highLookbackDays"
    | "minBodyPct" | "minRangeAtrRatio"
...
): PrecomputedEvsSignals {
  const result: PrecomputedEvsSignals = new Map();
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    for (const [ticker, bars] of allData) {
      ...
      // 20日高値を計算
      const lookback = config.highLookbackDays;
      const lookbackStart = Math.max(0, todayIdx - lookback);
      let high20 = 0;
      for (let j = lookbackStart; j < todayIdx; j++) {
        if (bars[j].high > high20) high20 = bars[j].high;
      }
      if (high20 <= 0) continue;

      // シグナル判定
      if (!isEarlyVolumeSpikeSignal({
        ...
        minHigh20Ratio: config.minHigh20Ratio,
        maxHigh20Ratio: config.maxHigh20Ratio,
        minBodyPct: config.minBodyPct,
        minRangeAtrRatio: config.minRangeAtrRatio,
      })) continue;
      ...
      const high20Ratio = todayBar.close / high20;
      const bodyPct = todayBar.close / todayBar.open - 1;
      ...
      daySignals.push({
        ...
        high20Ratio: Math.round(high20Ratio * 1000) / 1000,
        bodyPct: Math.round(bodyPct * 10000) / 10000,
      });
    }

    if (daySignals.length > 0) {
      // volumeSurgeRatio 降順（出来高サージが強いほど優先）
...
/**
 * Early Volume Spike バックテストを実行する
 */
export function runEvsBacktest(
  config: EarlyVolumeSpikeBacktestConfig,
  allData: Map<string, OHLCVData[]>,
  vixData?: Map<string, number>,
  indexData?: Map<string, number>,
  precomputed?: PrecomputedSimData,
  precomputedSignals?: PrecomputedEvsSignals,
): EarlyVolumeSpikeBacktestResult {
  ...
  if (precomputed) {
    ...
    precomputedSignals = precomputeEvsDailySignals(config, allData, computed);
  }
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (todayRegime !== "crisis" && openPositions.length < config.maxPositions) {
      ...
      for (const signal of signals) {
        ...
        const riskAmount = cash * (EVS_RISK_PER_TRADE_PCT / 100);
        ...
        if (config.verbose) {
          console.log(
            ...
            ` (vol${signal.volumeSurgeRatio.toFixed(1)}x, h20=${(signal.high20Ratio * 100).toFixed(1)}%, body${(signal.bodyPct * 100).toFixed(1)}%, SL¥${stopLossPrice})`,
          );
        }
      }
    }
