/**
 * ギャップダウンリバーサルバックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:gapdown-reversal
 *   npm run backtest:gapdown-reversal -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:gapdown-reversal -- --verbose
 */

import dayjs from "dayjs";
import { prisma } from "../lib/prisma";
import { GAPDOWN_REVERSAL_BACKTEST_DEFAULTS } from "./gapdown-reversal-config";
import { getMaxBuyablePrice } from "../core/risk-manager";
import { runGapDownReversalBacktest } from "./gapdown-reversal-simulation";
...
import type { GapDownReversalBacktestConfig, PerformanceMetrics } from "./types";
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? GAPDOWN_REVERSAL_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("ギャップダウンリバーサル バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: GapDownReversalBacktestConfig = {
    ...GAPDOWN_REVERSAL_BACKTEST_DEFAULTS,
    startDate,
    endDate,
  ...
  // デフォルト実行
  const result = runGapDownReversalBacktest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "ギャップダウンリバーサル戦略");
  ...
  // DBに保存
  try {
    const id = await saveBacktestResult(result, "gapdown-reversal");
...
main().catch((err) => {
  console.error("ギャップダウンリバーサルBTエラー:", err);
  process.exit(1);
});
