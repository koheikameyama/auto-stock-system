/**
 * NR7ブレイクバックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:nr7
 *   npm run backtest:nr7 -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:nr7 -- --verbose
 */

import dayjs from "dayjs";
import { prisma } from "../lib/prisma";
import { NR7_BACKTEST_DEFAULTS } from "./nr7-config";
import { getMaxBuyablePrice } from "../core/risk-manager";
import { runNR7Backtest } from "./nr7-simulation";
...
import type { NR7BacktestConfig, PerformanceMetrics } from "./types";
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? NR7_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("NR7ブレイク（Narrow Range 7 Breakout）バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: NR7BacktestConfig = {
  ...
  // デフォルト実行
  const result = runNR7Backtest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "NR7ブレイク（Narrow Range 7）戦略");
  ...
  // DBに保存
  try {
    const id = await saveBacktestResult(result, "nr7");
