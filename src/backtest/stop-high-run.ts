/**
 * ストップ高フォロースルーバックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:stop-high
 *   npm run backtest:stop-high -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:stop-high -- --verbose
 */
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? STOP_HIGH_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("ストップ高フォロースルー（Stop-High Follow-Through）バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: StopHighBacktestConfig = {
  ...
  const result = runStopHighBacktest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "ストップ高フォロースルー戦略");
...
main().catch((err) => {
  console.error("Stop-High BTエラー:", err);
  process.exit(1);
});
