/**
 * Down-Day Reversal バックテスト・シミュレーションエンジン
 *
 * 日次ループで Down-Day Reversal シグナルを検出し、当日終値でエントリー。
 * エグジットは既存の checkPositionExit() を再利用。
 *
 * マーケットフィルターは通常戦略と逆転:
 *   - breadth < maxBreadth (弱相場でのみエントリー)
 *   - index BELOW SMA50 (下落トレンドでのみエントリー)
 */
...
import { DDR_RISK_PER_TRADE_PCT } from "./down-day-reversal-config";
import { isDownDayReversalSignal } from "../core/down-day-reversal/entry-conditions";
...
import type {
  DownDayReversalBacktestConfig,
  DownDayReversalBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
...
/**
 * Down-Day Reversal シグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedDdrSignal {
  ticker: string;
  entryPrice: number;
  atr14: number;
...
export type PrecomputedDdrSignals = Map<string, PrecomputedDdrSignal[]>;

/**
 * Down-Day Reversal シグナルを一括事前計算する。
 *
 * マーケットフィルターは逆転:
 *   - breadth < maxBreadth (弱相場でのみエントリー)
 *   - index BELOW SMA50 (下落トレンドでのみエントリー)
 */
export function precomputeDdrDailySignals(
  config: Pick<DownDayReversalBacktestConfig,
    | "maxPrice" | "minAtrPct" | "minAvgVolume25" | "minTurnover" | "minPrice"
    | "minDropPct" | "volSurgeRatio" | "minBodyPct" | "maxBreadth"
    | "indexTrendFilter" | "indexTrendOffBufferPct" | "indexTrendOnBufferPct"
  >,
  allData: Map<string, OHLCVData[]>,
  precomputed: PrecomputedSimData,
): PrecomputedDdrSignals {
  const result: PrecomputedDdrSignals = new Map();
  const { tradingDays, dateIndexMap, dailyBreadth, dailyIndexAboveSma } = precomputed;

  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    const today = tradingDays[dayIdx];

    // マーケットフィルター（逆転）: breadth >= maxBreadth ならスキップ（強い相場ではエントリーしない）
    if ((dailyBreadth.get(today) ?? 1) >= config.maxBreadth) continue;

    // インデックスフィルター（逆転）: index が SMA50 の上にあればスキップ（上昇トレンドではエントリーしない）
    if (config.indexTrendFilter && dailyIndexAboveSma.get(today)) continue;

    const daySignals: PrecomputedDdrSignal[] = [];
    ...
    for (const [ticker, bars] of allData) {
      ...
      const dropPct = (todayBar.low - prevBar.close) / prevBar.close;
...
/**
 * Down-Day Reversal バックテストを実行する
 */
export function runDdrBacktest(
  config: DownDayReversalBacktestConfig,
  allData: Map<string, OHLCVData[]>,
  vixData?: Map<string, number>,
  indexData?: Map<string, number>,
  precomputed?: PrecomputedSimData,
  precomputedSignals?: PrecomputedDdrSignals,
): DownDayReversalBacktestResult {
  ...
  if (precomputed) {
    ...
    const computed = precomputeSimData(
      ...
      true, // marketTrendFilter always enabled for DDR (breadth filter)
    ...
    precomputedSignals = precomputeDdrDailySignals(config, allData, computed);
  }
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (todayRegime !== "crisis" && openPositions.length < config.maxPositions) {
      ...
      for (const signal of signals) {
        ...
        const riskAmount = cash * (DDR_RISK_PER_TRADE_PCT / 100);
        ...
        if (config.verbose) {
          console.log(
            ...
            ` (drop${(signal.dropPct * 100).toFixed(1)}%, body${(signal.bodyPct * 100).toFixed(1)}%, vol${signal.volumeSurgeRatio.toFixed(1)}x, SL¥${stopLossPrice})`,
          );
        }
      }
    }
