/**
 * MA押し目買いバックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:ma-pullback
 *   npm run backtest:ma-pullback -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:ma-pullback -- --verbose
 */
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? MA_PULLBACK_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("MA押し目買い バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: MaPullbackBacktestConfig = {
  ...
  // デフォルト実行
  const result = runMaPullbackBacktest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "MA押し目買い戦略");
...
main().catch((err) => {
  console.error("MA押し目買いBTエラー:", err);
  process.exit(1);
});
