...
import type {
  ...
  PostSurgeConsolidationBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
 ...
 * PSCシグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedPSCSignal {
  ticker: string;
  entryPrice: number;
...
export type PrecomputedPSCSignals = Map<string, PrecomputedPSCSignal[]>;

/**
 * PSCシグナルを一括事前計算する。
 * WFでは IS/OOS それぞれ1回呼んで全コンボに渡す。
 */
export function precomputePSCDailySignals(
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    for (const [ticker, bars] of allData) {
      ...
      // 20日ルックバック必須
      if (todayIdx < POST_SURGE_CONSOLIDATION.ENTRY.MOMENTUM_LOOKBACK_DAYS) continue;
      ...
      const bar20DaysAgo = bars[todayIdx - POST_SURGE_CONSOLIDATION.ENTRY.MOMENTUM_LOOKBACK_DAYS];
      if (!bar20DaysAgo) continue;
      ...
      const momentumReturn = (todayBar.close / bar20DaysAgo.close) - 1;
      ...
      daySignals.push({
        ticker,
        entryPrice,
        momentumReturn: Math.round(momentumReturn * 10000) / 10000,
        atr14,
...
export function runPSCBacktest(
  ...
  if (precomputed) {
    ...
    // シグナルも計算
    precomputedSignals = precomputePSCDailySignals(config, allData, computed);
  }
      ...
      for (const signal of signals) {
        ...
        if (config.verbose) {
          console.log(
            ...
            ` (mom${(signal.momentumReturn * 100).toFixed(1)}%, vol${signal.volumeSurgeRatio.toFixed(1)}x, SL¥${stopLossPrice})`,
          );
        }
      }
    }
