/**
 * Early Volume Spike バックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:evs
 *   npm run backtest:evs -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:evs -- --verbose
 */

import dayjs from "dayjs";
import { prisma } from "../lib/prisma";
import { EVS_BACKTEST_DEFAULTS } from "./early-volume-spike-config";
import { getMaxBuyablePrice } from "../core/risk-manager";
import { runEvsBacktest } from "./early-volume-spike-simulation";
...
import type { EarlyVolumeSpikeBacktestConfig, PerformanceMetrics } from "./types";
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? EVS_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("Early Volume Spike バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: EarlyVolumeSpikeBacktestConfig = {
    ...EVS_BACKTEST_DEFAULTS,
    startDate,
    endDate,
  ...
  const result = runEvsBacktest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "Early Volume Spike 戦略");
  ...
  try {
    const id = await saveBacktestResult(result, "early-volume-spike");
