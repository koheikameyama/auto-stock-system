 ...
 *
 * Usage:
 *   npm run backtest:earnings-gap
 *   npm run backtest:earnings-gap -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:earnings-gap -- --verbose
 */

import dayjs from "dayjs";
import { prisma } from "../lib/prisma";
import { EARNINGS_GAP_BACKTEST_DEFAULTS } from "./earnings-gap-config";
import { getMaxBuyablePrice } from "../core/risk-manager";
import { runEarningsGapBacktest } from "./earnings-gap-simulation";
import { fetchHistoricalFromDB, fetchVixFromDB, fetchIndexFromDB, fetchEarningsFromDB } from "./data-fetcher";
import { calculateCapitalUtilization } from "./metrics";
import type { EarningsGapBacktestConfig, PerformanceMetrics } from "./types";
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? EARNINGS_GAP_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("決算ギャップ バックテスト");
  console.log("=".repeat(60));
  ...
  const earningsData = await fetchEarningsFromDB(tickerCodes, startDate, endDate);
  console.log(`[data] ${rawData.size}銘柄, VIX ${vixData.size}日, N225 ${indexData.size}日, 決算日 ${earningsData.size}銘柄`);
  ...
  const baseConfig: EarningsGapBacktestConfig = {
    ...EARNINGS_GAP_BACKTEST_DEFAULTS,
    startDate,
    endDate,
  ...
  // 実行
  const result = runEarningsGapBacktest(baseConfig, allData, earningsData, vixArg, indexArg);
  printResult(result, "決算ギャップ戦略");
