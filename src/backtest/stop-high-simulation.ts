/**
 * ストップ高フォロースルーバックテスト・シミュレーションエンジン
 *
 * 日次ループで ストップ高フォロースルー シグナルを検出し、当日終値でエントリー。
...
import { getLimitDownPrice, getLimitUpPrice } from "../lib/constants/price-limits";
...
import { STOP_HIGH_RISK_PER_TRADE_PCT } from "./stop-high-config";
import { isStopHighSignal } from "../core/stop-high/entry-conditions";
...
import type {
  StopHighBacktestConfig,
  StopHighBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
...
/**
 * ストップ高フォロースルー シグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedStopHighSignal {
  ticker: string;
  entryPrice: number;
  atr14: number;
...
export type PrecomputedStopHighSignals = Map<string, PrecomputedStopHighSignal[]>;

/**
 * ストップ高シグナルを一括事前計算する。
 */
export function precomputeStopHighDailySignals(
  config: Pick<StopHighBacktestConfig,
    | "maxPrice" | "minAtrPct" | "minAvgVolume25" | "minTurnover" | "minPrice"
    | "stopHighThresholdRatio" | "minGapPct" | "minBodyPct" | "maxBodyPct" | "volSurgeRatio"
...
): PrecomputedStopHighSignals {
  const result: PrecomputedStopHighSignals = new Map();
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    const daySignals: PrecomputedStopHighSignal[] = [];
    ...
    for (const [ticker, bars] of allData) {
      ...
      const prevPrevBar = bars[todayIdx - 2];
      if (!todayBar || !prevBar || !prevPrevBar) continue;
      ...
      const prevStopHighPrice = getLimitUpPrice(prevPrevBar.close);
      const todayStopHighPrice = getLimitUpPrice(prevBar.close);

      // シグナル判定
      if (!isStopHighSignal({
        prevPrevClose: prevPrevBar.close,
        ...
        avgVolume25,
        todayStopHighPrice,
        stopHighThresholdRatio: config.stopHighThresholdRatio,
        prevStopHighPrice,
        minGapPct: config.minGapPct,
        minBodyPct: config.minBodyPct,
        maxBodyPct: config.maxBodyPct,
...
/**
 * ストップ高フォロースルーバックテストを実行する
 */
export function runStopHighBacktest(
  config: StopHighBacktestConfig,
  allData: Map<string, OHLCVData[]>,
  vixData?: Map<string, number>,
  indexData?: Map<string, number>,
  precomputed?: PrecomputedSimData,
  precomputedSignals?: PrecomputedStopHighSignals,
): StopHighBacktestResult {
  ...
  if (precomputed) {
    ...
    precomputedSignals = precomputeStopHighDailySignals(config, allData, computed);
  }
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (todayRegime !== "crisis" && openPositions.length < config.maxPositions) {
      ...
      for (const signal of signals) {
        ...
        const riskAmount = cash * (STOP_HIGH_RISK_PER_TRADE_PCT / 100);
        ...
        if (config.verbose) {
          console.log(
            ...
            ` (gap${(signal.gapPct * 100).toFixed(1)}%, body${(signal.bodyPct * 100).toFixed(1)}%, vol${signal.volumeSurgeRatio.toFixed(1)}x, SL¥${stopLossPrice})`,
          );
        }
      }
    }
