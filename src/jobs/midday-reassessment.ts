...
${sectorText || "  特になし"}`;
  ...
  for (const order of pendingBuyOrders) {
    ...
    if (order.strategy === "day_trade") {
      ...
      order.strategy === "breakout" &&
      DEFENSIVE_MODE.ENABLED_SENTIMENTS.includes(effectiveSentiment)
    ) {
      // bearish/crisis環境ではブレイクアウト新規買いもキャンセル
      shouldCancel = true;
      cancelReason = `${effectiveSentiment}環境でのブレイクアウト買い注文キャンセル`;
    } else if (
      order.strategy === "breakout" &&
      effectiveSentiment === "cautious"
    ) {
      // cautious環境: breakout注文はday_tradeパラメータと不整合のためキャンセル
      shouldCancel = true;
      cancelReason = "cautious環境でのブレイクアウト買い注文キャンセル（day_trade切替のため）";
    }
