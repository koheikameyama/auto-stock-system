...
export async function main() {
  ...
  // 1.6. pending swing注文のテーシス無効化チェック
  const pendingSwingOrders = pendingBuyOrders.filter(
    (o) => o.strategy === "swing" && o.status === "pending",
  );

  if (pendingSwingOrders.length > 0) {
    console.log(
      `\n  [テーシス無効化チェック] pending swing注文: ${pendingSwingOrders.length}件`,
    );

    const thesisLimit = pLimit(THESIS_INVALIDATION.CONCURRENCY);
    const invalidated: Array<{ id: string; tickerCode: string; name: string; reason: string }> = [];
          ...
          try {
            ...
            if (stopLossPrice != null && currentPrice < stopLossPrice) {
              invalidated.push({
                ...
                tickerCode,
                name: stockName,
                reason: `損切りライン割れ（現在値 ¥${currentPrice.toLocaleString()} < SL ¥${stopLossPrice.toLocaleString()}）`,
              });
              return;
            }
            ...
            if (
              ...
              techSummary.deviationRate25 < THESIS_INVALIDATION.DEVIATION_RATE_25_THRESHOLD
            ) {
              invalidated.push({
                id: order.id,
                tickerCode,
    ...
    if (invalidated.length > 0) {
      ...
      for (const r of invalidated) {
        console.log(`  [${r.tickerCode}] テーシス無効化でキャンセル: ${r.reason}`);
      }
      ...
      await notifySlack({
        ...
        color: "warning",
        fields: [
          { title: "キャンセル数", value: `${invalidated.length}件`, short: true },
          {
            title: "残存swing注文",
    ...
    console.log(
      `  [テーシス無効化チェック] 完了: ${invalidated.length}件キャンセル / ${pendingSwingOrders.length - invalidated.length}件継続`,
    );
  }
