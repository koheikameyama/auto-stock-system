 ...
 * 前場MA押し目スキャナー
 *
 * 前場（9:00-11:30 JST）中に5分ごと呼び出され、ウォッチリスト銘柄の
 * 現在価格が20日MAにタッチ（±2%以内）し、かつ上昇中の場合にシグナルを記録する。
 *
 * 【実行頻度】5分おき（9:00, 9:05, ..., 11:30）。立花API高負荷警告対応として
 * 毎分 → 5分おきに削減（2026-04-22）。本ジョブは「前回タッチから5分以上経過」
 * のノイズ排除制約を持つため、5分おきでも情報量は維持される。
 *
 * リタッチ対応:
 * - 初回タッチ → 新規レコード作成
