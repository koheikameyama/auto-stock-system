...
① ニュース・カタリスト（最優先）
  ...
  - 【ニュース】セクションがない銘柄 → ニュートラル扱い（ニュースがないこと自体はリスクではない）
② 地政学・マクロリスク
  - 市場評価のセンチメントとの整合性
  - 地政学イベント（戦争・制裁・選挙等）の影響を受けるセクターか
③ セクター全体の流れ
  - 【リスクコンテキスト】に記載されたセクター名を必ず正確に参照すること（例: 「素材」セクターの銘柄を「半導体」と書かない）
...
- 【厳守】提供された情報（【ニュース】【リスクコンテキスト】セクション）に記載されていない事実を根拠にしないでください。「訴訟リスクがある」「不祥事が報じられている」等、提供データに存在しないニュースを理由にNo-Goにしてはいけません
