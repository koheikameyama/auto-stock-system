...
export async function notifyBrokerOrderSubmitted(data: {
  ...
  await notifySlack({
    title: `ブローカー注文送信: ${data.ticker} [${data.side.toUpperCase()}]`,
...
/** 週次戦略ヘルスチェック通知 */
export async function notifyHealthCheck(data: {
  date: string;
  strategies: Array<{
    name: string;
    pf6m: number; pf3m: number;
    winRate6m: number; winRate3m: number;
    expectancy6m: number; expectancy3m: number;
    trades6m: number; trades3m: number;
    maxDD6m: number; maxDD3m: number;
    status: string;
    statusReason: string;
  }>;
}): Promise<void> {
  const fmtPf = (pf: number) => (pf === Infinity || pf >= 9999 ? "∞" : pf.toFixed(2));
  const fmtExp = (e: number) => (e >= 0 ? "+" : "") + e.toFixed(2) + "%";
  const statusIcon = (s: string) => s === "stable" ? "✓" : s === "warning" ? "△" : "✗";

  const lines: string[] = [];
  for (const s of data.strategies) {
    lines.push(`*${s.name}* [${statusIcon(s.status)}]`);
    lines.push(`  6mo: PF ${fmtPf(s.pf6m)} | ${s.winRate6m.toFixed(1)}% | ${fmtExp(s.expectancy6m)} | DD -${s.maxDD6m.toFixed(1)}% | ${s.trades6m}tr`);
    lines.push(`  3mo: PF ${fmtPf(s.pf3m)} | ${s.winRate3m.toFixed(1)}% | ${fmtExp(s.expectancy3m)} | DD -${s.maxDD3m.toFixed(1)}% | ${s.trades3m}tr`);
    if (s.statusReason) {
      lines.push(`  => ${s.statusReason}`);
    }
  }

  const hasIssue = data.strategies.some((s) => s.status !== "stable");

  await notifySlack({
    title: hasIssue ? "△ Strategy Health Check" : "✓ Strategy Health Check",
    message: lines.join("\n"),
