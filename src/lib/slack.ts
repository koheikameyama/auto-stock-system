...
export async function notifyMarketForecast(data: {
  ...
  tradingHints?: string;
  vix: number | null;
  nikkeiPrice: number | null;
  usdjpy: number | null;
}): Promise<void> {
  const outlookEmoji: Record<string, string> = { bullish: "🟢", neutral: "🟡", bearish: "🔴" };
  const outlookLabel: Record<string, string> = { bullish: "強気", neutral: "中立", bearish: "弱気" };
  const stars = "★".repeat(data.confidence) + "☆".repeat(5 - data.confidence);

  const factorsText = data.keyFactors
    .map((f) => `• ${f.factor}（${f.impact}）`)
    .join("\n");
  const risksText = data.risks
  ...
  const emoji = outlookEmoji[data.outlook] ?? "⚪";
  const label = outlookLabel[data.outlook] ?? data.outlook;

  await notifySlack({
    title: `${emoji} 市場予想（${data.targetDate}）: ${label} ${stars}`,
    message,
    color: data.outlook === "bullish" ? "good" : data.outlook === "bearish" ? "danger" : "warning",
    fields: [
      { title: "VIX", value: data.vix != null ? `${data.vix.toFixed(1)}` : "N/A", short: true },
      { title: "日経225", value: data.nikkeiPrice != null ? `¥${data.nikkeiPrice.toLocaleString()}` : "N/A", short: true },
      { title: "USD/JPY", value: data.usdjpy != null ? `${data.usdjpy.toFixed(2)}` : "N/A", short: true },
      { title: "確信度", value: stars, short: true },
    ],
  });
}
