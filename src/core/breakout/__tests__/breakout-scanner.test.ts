...
describe("BreakoutScanner", () => {
  ...
  it("1. 初期状態: hotSet が空でトリガーが発火しない", () => {
    ...
    const triggers = scanner.scan([quote], SCAN_TIME, DAILY_ENTRY_COUNT, NO_HOLDINGS);
  ...
  it("3. surgeRatio < 1.2 が2回連続 → Hot から Cold に降格する", () => {
    ...
    scanner.scan([makeQuote(DEFAULT_TICKER, 1.0, 9, 31)], time2, DAILY_ENTRY_COUNT, NO_HOLDINGS);
    ...
    scanner.scan([makeQuote(DEFAULT_TICKER, 1.0, 9, 32)], time3, DAILY_ENTRY_COUNT, NO_HOLDINGS);
  ...
  it("4. surgeRatio < 1.2 の1回後に >= 1.2 → coolDownCount がリセットされる", () => {
    ...
    const time3 = makeTime(9, 32);
    scanner.scan([makeQuote(DEFAULT_TICKER, 1.3, 9, 32)], time3, DAILY_ENTRY_COUNT, NO_HOLDINGS);
    expect(scanner.getState().hotSet.has(DEFAULT_TICKER)).toBe(true);
    expect(scanner.getState().hotSet.get(DEFAULT_TICKER)!.coolDownCount).toBe(0);
  });
  ...
  it("5. surgeRatio >= 2.0 かつ price > high20 → トリガーが発火する", () => {
    ...
    const triggers = scanner.scan([triggerQuote], time2, DAILY_ENTRY_COUNT, NO_HOLDINGS);
  ...
  it("6. 同一ティッカーの2回目トリガー → triggeredToday でブロックされる", () => {
    ...
    const firstTriggers = scanner.scan([triggerQuote], time2, DAILY_ENTRY_COUNT, NO_HOLDINGS);
    ...
    const secondTriggers = scanner.scan([triggerQuote], time3, DAILY_ENTRY_COUNT, NO_HOLDINGS);
  ...
  it("7. 9:05 より前 → スキャンを行わない（Cold 昇格もトリガーもなし）", () => {
    ...
    const triggers = scanner.scan([quote], earlyTime, DAILY_ENTRY_COUNT, NO_HOLDINGS);

    expect(triggers).toHaveLength(0);
    expect(scanner.getState().hotSet.size).toBe(0);
  });

  // 8. After 14:30 → no trigger firing
  it("8. 14:30 より後 → トリガーが発火しない（Hot 昇格は可能）", () => {
    // First promote to hot at 9:30
    scanner.scan([makeQuote(DEFAULT_TICKER, 1.5)], SCAN_TIME, DAILY_ENTRY_COUNT, NO_HOLDINGS);
    expect(scanner.getState().hotSet.has(DEFAULT_TICKER)).toBe(true);

    // After 14:30, attempt to trigger
    const lateTime = makeTime(14, 31);
    const triggerQuote: QuoteData = {
      ticker: DEFAULT_TICKER,
      price: 1010,
      volume: Math.ceil(volumeForRatio(2.5, 100_000, 14, 31)),
    };
    const triggers = scanner.scan([triggerQuote], lateTime, DAILY_ENTRY_COUNT, NO_HOLDINGS);

    expect(triggers).toHaveLength(0);
  });

  // 9. Daily entry limit (3) reached → no trigger firing
  it("9. 1日のエントリー上限（3件）到達 → トリガーが発火しない", () => {
    // Promote to hot
    scanner.scan([makeQuote(DEFAULT_TICKER, 1.5)], SCAN_TIME, DAILY_ENTRY_COUNT, NO_HOLDINGS);
    ...
    // dailyEntryCount = 3 (at limit)
    const triggers = scanner.scan([triggerQuote], time2, 3, NO_HOLDINGS);

    expect(triggers).toHaveLength(0);
  });

  // 10. Holding tickers → skipped
  it("10. 保有中のティッカー → Cold スキャンをスキップし、トリガーも発火しない", () => {
    ...
    const triggers = scanner.scan([quote], SCAN_TIME, DAILY_ENTRY_COUNT, holdings);
  ...
  it("11. Cold スキャンは各ティッカーで5分ごとにのみ実行される", () => {
    ...
    const quote = makeQuote(DEFAULT_TICKER, 1.5);
    scanner.scan([quote], SCAN_TIME, DAILY_ENTRY_COUNT, NO_HOLDINGS);
    expect(scanner.getState().hotSet.has(DEFAULT_TICKER)).toBe(true);
    ...
    scanner.scan([highSurgeQuote], time2, DAILY_ENTRY_COUNT, NO_HOLDINGS);
